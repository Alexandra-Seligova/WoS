...
{
    public class DronDeluxeAlpha : DroneBase
    {
        private const float SCALE_FACTOR = 0.1f; // 10% z původní velikosti

        public DronDeluxeAlpha(ContentManager content, Vector2 startPosition) : base(content, startPosition)
        {
            Texture = content.Load<Texture2D>("drons/DronDeluxeAlpha");
            Hp = 700;
            HpMax = 700;
            MaxSpeed = 500;
            Shield = 0;
